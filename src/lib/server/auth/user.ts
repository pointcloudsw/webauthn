...
export function getUserFromGoogleId(idpId: string): User | null {
	const row = db.queryOne("SELECT id, google_id, email, name, picture FROM user WHERE google_id = ?", [idpId]);
	if (row === null) {
		return null;
	}
	const user: User = {
