...
export function validateSessionToken(token: string): SessionValidationResult {
	...
	// let etok = te.encode(token);
	// let stok = sha256(etok);
	// if ( !stok ) return { session: null, user: null };
	// const sessionId = encodeHexLowerCase(stok);
...
// export function getSessionTokenCookie({ event }: { event: RequestEvent; }): string {
...
// export function getUserId({ event }: { event: RequestEvent; }): number | undefined {
