...
async function action(event: RequestEvent) {
	...
	console.log('[116]authenticatorData',authenticatorData);
	// if (!authenticatorData.verifyRelyingPartyIdHash(new URL(import.meta.env.BASE_URL).hostname)) {
	if (!authenticatorData.verifyRelyingPartyIdHash("www1.pcpllc.us")) {
		return fail(400, {
			message: "Invalid data 119"
		});
	}
	...
	console.log('[153]ClientData.Origin',clientData.origin);
	// if (clientData.origin !== import.meta.env.BASE_URL) {
	if (clientData.origin !== "https://www1.pcpllc.us") {
		return fail(400, {
			message: "Invalid data 157"
		});
	}
